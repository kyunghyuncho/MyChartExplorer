...
st.markdown(
    ...
    - If you select a hosted AI provider (OpenRouter), prompts and small previews of your data may be transmitted to OpenRouter using your API key and are governed by OpenRouter's terms. We currently limit hosted usage to the model `google/gemini-3.1-flash-lite-preview` via OpenRouter. As of Oct 5, 2025, Google's Gemini API policy states requests are not used to train Google's models by default; see ai.google.dev/gemini-api/terms. OpenRouter's policies also apply. If you select a local provider (Ollama), no prompts leave your environment.
