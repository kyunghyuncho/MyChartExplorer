...
with st.form("settings_form"):
    ...
    try:
        ...
        try:
            ...
            provisioned = None
        saved_key = config.get("openrouter_api_key", "")
        # Cleanup: if the user accidentally saved the placeholder text as their key, clear it
        if saved_key == "(managed by admin - type to override)":
            saved_key = ""
        ...
        openrouter_api_key = st.text_input(
            ...
            type="password",
            value=saved_key,
            placeholder="(managed by admin - type to override)" if provisioned else "",
        ...
        or_models = ["google/gemini-3.1-flash-lite-preview", 
                     "google/gemini-3-flash-preview", 
                     "google/gemini-3.1-pro-preview"]
        cur_or_model = config.get("openrouter_model", "google/gemini-3.1-flash-lite-preview")
        if cur_or_model not in or_models:
            cur_or_model = "google/gemini-3.1-flash-lite-preview"
        openrouter_model = st.selectbox(
            "OpenRouter Model",
            or_models,
            index=or_models.index(cur_or_model),
            help="Select the Gemini model to use with OpenRouter."
        )

        st.caption(
            f"Hosted provider: OpenRouter. Selected model: {openrouter_model}. "
            "When this provider is selected, prompts are sent to OpenRouter using your key."
        )
        st.caption(
            "Policy note (Oct 5, 2025): We currently limit hosted usage to specific Gemini models. "
    ...
    if submitted and not form_error:
        ...
        new_config = {
            ...
            # If admin manages a key, allow user to override it (or clear it to fallback)
            "openrouter_api_key": config.get("openrouter_api_key") if provisioned and openrouter_api_key == "(managed by admin - type to override)" else openrouter_api_key,
        ...
        if (openrouter_api_key or "") != (disk_after.get("openrouter_api_key") or ""):
            if not provisioned or openrouter_api_key != "(managed by admin - type to override)":
        ...
        if (openrouter_model or "") != (disk_after.get("openrouter_model") or ""):
            mismatches.append("OpenRouter Model")
