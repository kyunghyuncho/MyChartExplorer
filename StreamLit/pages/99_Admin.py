...
with tab_email:
    st.subheader("Email (Resend)")
    sg = st.text_input("Resend API Key", type="password", value=get_resend_api_key(), key="email_resend_key")
    if st.button("Save Resend Key", key="email_save_resend"):
        set_resend_api_key(sg)
        st.success("Resend API key saved.")
